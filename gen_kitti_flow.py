...
if __name__ == '__main__':
    ...
    while len(l) > 0:
        ...
        # for forward scene flow
        points1 = dataset_raw.get_lidar(raw_date, raw_sequence_id, raw_frame_id)[:, :3]
        points_mid = dataset_raw.get_lidar(raw_date, raw_sequence_id, raw_frame_id + 1)[:, :3]
        points2 = dataset_raw.get_lidar(raw_date, raw_sequence_id, raw_frame_id + 2)[:, :3]
        points3 = dataset_raw.get_lidar(raw_date, raw_sequence_id, raw_frame_id + 4)[:, :3]
        points4 = dataset_raw.get_lidar(raw_date, raw_sequence_id, raw_frame_id + 6)[:, :3]

        # for forback scene flow
        # points2 = dataset_raw.get_lidar(raw_date, raw_sequence_id, raw_frame_id)[:, :3]
        # points_mid = dataset_raw.get_lidar(raw_date, raw_sequence_id, raw_frame_id + 1)[:, :3]
        # points1 = dataset_raw.get_lidar(raw_date, raw_sequence_id, raw_frame_id + 2)[:, :3]
        # points3 = dataset_raw.get_lidar(raw_date, raw_sequence_id, raw_frame_id - 2)[:, :3]
        # points4 = dataset_raw.get_lidar(raw_date, raw_sequence_id, raw_frame_id - 4)[:, :3]

        points1 = points1[points1[:, 0] > 0]
        points_mid = points_mid[points_mid[:, 0] > 0]
        ...
        project_pos_mid = project(P_velo_to_img[raw_date], points_mid)
        fov_inds = (np.round(project_pos_mid[:,1]) < disp0.shape[0]) & (np.round(project_pos_mid[:,1]) >= 0) & \
                    (np.round(project_pos_mid[:,0]) < disp0.shape[1]) & (np.round(project_pos_mid[:,0]) >= 0)
        project_pos_mid = project_pos_mid[fov_inds]
        points_mid = points_mid[fov_inds]
        ...
        points_mid_cam_0_coord = transform(Tr_velo_to_cam[raw_date], points_mid)
        points_mid_rect = transform(R_cam_to_rect[raw_date], points_mid_cam_0_coord)
        ...
        print('avg Time: ', (stop_time - start_time)/frame_count) 
