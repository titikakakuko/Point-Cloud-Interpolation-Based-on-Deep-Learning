...
def attention(query, key, value, mask=None, dropout=None):
    d_k = query.size(-1)
    # print(query.size(), key.transpose(-2, -1).contiguous().size())
    # z = torch.bmm(x.unsqueeze(0).expand(192, *x.shape), y)
    # with torch.no_grad():
    #     q = query.resize_((1, 4, 20329, 1))
    #     k = key.resize_((1, 4, 20329, 1))
    # scores = torch.matmul(q, k.transpose(-2, -1).contiguous()) / math.sqrt(d_k)

    # q = query.transpose(-1, 0).contiguous()[0]
    # k = key.transpose(-1, 0).contiguous()[0]
    # v = value.transpose(-2, -1).contiguous()[0][0][0]
    # print(q.size(), k.transpose(-2, -1).contiguous().size())
    # scores = torch.matmul(q, k) / math.sqrt(d_k)

    scores = torch.matmul(query, key.transpose(-2, -1).contiguous()) / math.sqrt(d_k)
    # scores = torch.bmm(query.unsqueeze(0).expand_as(key.transpose(-2, -1).contiguous()), key.transpose(-2, -1).contiguous())
    if mask is not None:
        scores = scores.masked_fill(mask == 0, -1e9)
    p_attn = F.softmax(scores, dim=-1)
    # return p_attn* value, p_attn
    return torch.matmul(p_attn, value), p_attn
    # return torch.matmul(p_attn, v.transpose(-1, 0).contiguous()), p_attn
...
def get_graph_feature(x, k=20):
    ...
    # print(idx.is_cuda,idx_base.is_cuda)
    ...
    feature = torch.cat((feature, x), dim=3).permute(0, 3, 1, 2).to(device).type(torch.FloatTensor)
...
class SVDHead(nn.Module):
    ...
    def forward(self, *input):
        ...
        tgt=torch.where(torch.isnan(tgt), torch.full_like(tgt, 0), tgt)

        # scores = torch.tensor(scores, dtype=torch.float32)
        src_corr = torch.matmul(tgt.cuda(), scores.transpose(2, 1).contiguous())
        src_corr=torch.where(torch.isnan(src_corr), torch.full_like(src_corr, 0), src_corr)
        
        src_centered = src - src.mean(dim=2, keepdim=True)

        src_corr_centered = src_corr - src_corr.mean(dim=2, keepdim=True)

        src_centered = torch.tensor(src_centered, dtype=torch.float32)
        src_corr_centered = torch.tensor(src_corr_centered, dtype=torch.float32)
        # print(src_centered,src_corr_centered)

        H = torch.matmul(src_centered.cuda(), src_corr_centered.cuda().transpose(2, 1).contiguous())
        ...
        t = torch.matmul(-R.cuda(), src.mean(dim=2, keepdim=True).cuda()) + src_corr.mean(dim=2, keepdim=True)
